...
tx_hash = client.deploy_contract(code=code, account=acct, args=[], leader_only=False)
print(f"TX: {tx_hash}")
...
for i in range(60):
    ...
    try:
        ...
        msg = str(e)
        if "KeyError" not in msg and "status" not in msg.lower() and "timeout" not in msg.lower():
            print(f"Error: {msg[:100]}")
            break
        import requests
        r = requests.post("https://studio.genlayer.com/api",
            json={"jsonrpc":"2.0","method":"eth_getTransactionReceipt","params":[tx_hash],"id":1}, timeout=10)
